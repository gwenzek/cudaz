...
const ShmemReduce = cuda_module.KernelSignature(ptx_file, "shmem_reduce_kernel");
const GlobalReduce = cuda_module.KernelSignature(ptx_file, "global_reduce_kernel");
...
fn reduce(cuda: *Cuda, reduce_kernel: GlobalReduce, d_out: []f32, d_intermediate: []f32, d_in: []f32, uses_shared_memory: bool) !f32 {
    ...
    if (size % maxThreadsPerBlock != 0 or size > maxThreadsPerBlock * maxThreadsPerBlock) {
        ...
        return cuda_module.CudaError.InvalidValue;
    }
    const full_grid = cuda_module.Grid{
    ...
    const one_block = cuda_module.Grid{ .blockDim = .{}, .threadDim = full_grid.blockDim };
...
pub fn main() !void {
    ...
    var shared_sum = try benchmark_reduce(&cuda, 100, d_out, d_intermediate, d_in, true);
    var global_sum = try benchmark_reduce(&cuda, 100, d_out, d_intermediate, d_in, false);
}
