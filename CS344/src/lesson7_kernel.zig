const cu = @import("cuda").cu;
const ptx = @import("nvptx");

pub export fn quicksort(d_out: []f32) callconv(ptx.kernel) void {
    // TODO: this should be a "no-divergence point", mark it as such
    if (d_out.len == 1) return;

    const tid = ptx.getIdX();
    ...
    if (tid == 0 and pivot_id > 0) {
        // Left stream
        cu.cudaStreamCreateWithFlags(&sub_stream, cu.CU_STREAM_NON_BLOCKING);
